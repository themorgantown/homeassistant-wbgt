...
3. **Jurisdiction scoping (`_scope_composite`)** — the API's own `composite` is the single most-protective standard across *all 76 worldwide*, so an unrelated rule (e.g. the UAE midday work ban) can hijack guidance for a US user. The API has no server-side filter, so the coordinator recomputes the composite over only the standards relevant to the configured `country`/`state` (`_standard_in_scope`), using each standard's own `recommendation` from `results[]`. This is *selection*, not heat science — the per-standard numbers still come from the server. Country defaults to `hass.config.country`; a blank country scopes to global standards only. See `GLOBAL_JURISDICTIONS` / `US_STATE_JURISDICTIONS` in `const.py`.
4. The flattened response dict feeds 8 `CoordinatorEntity` sensors (`sensor.py`) + 1 binary sensor (`binary_sensor.py`). `_derive_risk_level()` maps the scoped composite to a risk tier locally.
